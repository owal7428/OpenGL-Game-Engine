...
void BoxCollider::CollisionTest(glm::vec3 playerPosition)
{
    ...
    for (int i = 0; i < size; i++)
    {
        glm::vec3 normal1 = glm::normalize(glm::cross(glm::vec3(1, 0, 0), normals[i]));
        glm::vec3 normal2 = glm::normalize(glm::cross(glm::vec3(0, 1, 0), normals[i]));
        glm::vec3 normal3 = glm::normalize(glm::cross(glm::vec3(0, 0, 1), normals[i]));

        if (!glm::all(glm::isnan(normal1)))
            axes.push_back(normal1);
        
        if (!glm::all(glm::isnan(normal2)))
            axes.push_back(normal2);
        
        if (!glm::all(glm::isnan(normal3)))
